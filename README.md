...
- 修复预约记录接口返回错误但被当作空列表的问题，查询失败时会显示明确原因。
- 修复命令行按开始时间取消预约时，找不到匹配记录仍提交 `recordId=None` 的问题。
- 优化 Gradio 开始预约流程：自动预热、登录或座位解析失败时保留当前预约列表，方便修正后重试。
...
说明：预热阶段只登录账号、解析座位映射并建立连接，不会提前提交真正的预约接口 `saveRecord`。如果自动预热或登录解析阶段失败，前端会保留当前预约列表，方便修正后重试。
