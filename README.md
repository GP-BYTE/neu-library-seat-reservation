...
一个用于成都东软学院图书馆座位预约的 Python 工具，支持 FastAPI 可视化界面、命令行和旧版 Gradio 界面。项目会在本地保存账号凭据和座位映射缓存，方便后续快速预约；开源仓库不会包含真实账号、密码、预约历史或 token。
...
- 修复 FastAPI 和 Gradio 开始预约流程中的成功误判：后台线程结束后会汇总每条提交结果，只有接口明确返回 `success` 才显示预约完成。
- 优化开抢瞬间的临时失败处理：遇到“频繁”“未开始”“未到时间”等临时返回时继续重试，并在日志中显示下一次重试。
- 预约失败或部分失败时不再清空全部待预约列表，未成功的预约会保留，便于直接重试。
- 当前预约记录查询后支持收起/展开记录卡片，记录较多时不会把页面撑得过长。
- 进一步优化前端轮询稳定性：空闲状态低频刷新，运行/预热时才保持较快日志刷新，输入框聚焦时暂停非必要刷新。
- 移除会造成 Safari 视觉跳动的输入框、按钮和卡片位移动画，当前预约列表更稳定。
- 为前端静态资源增加版本号和 no-cache 响应，降低 Safari 继续加载旧 CSS/JS 的概率。
- 优化电脑竖向窗口下的整体卡片排版，主内容区改为统一 dashboard 网格，减少分段卡片的杂乱感。
- 调整响应式断点，竖向桌面窗口保持更紧凑的两列表单，极窄手机窗口才切换为单列。
- 优化预约模板和历史记录区域布局，避免按钮、下拉框在 Safari 中挤压错位。
- 优化前端轮询渲染逻辑，预约列表、模板下拉、历史下拉和账号列表只有数据变化时才更新，避免当前预约卡片反复跳动。
...
- 新增 Apple Liquid Glass 风格前端，包含玻璃面板、状态动效、实时日志刷新和响应式布局。
- 前端不展示密码或编码后密码；预约列表只显示密码是否已缓存或本次已填写。
- 新增预约模板功能，可将当前多账号预约列表保存为本地模板，并在下次一键加载。
- 预约模板不会保存密码，加载后仍使用账号本地密码缓存或本次手动填写密码。
...
- 支持 Gradio 前端界面预约座位。
- 支持 FastAPI + Liquid Glass 新前端界面预约座位。
- 支持命令行交互预约和取消预约。
...
1. 在“预约控制台”添加多账号、多座位预约。
2. 使用“预热 / 检查”提前登录账号、解析座位映射和建立连接。
3. 使用“开始预约”启动后台预约任务，日志区会自动刷新运行状态。
4. 使用“模板与历史”保存或加载常用预约组合。
5. 使用“当前预约”按目标账号独立查询和取消预约。
6. 使用“账号管理”下线指定账号或一键下线全部账号。

说明：FastAPI 前端不会向页面返回真实密码或编码后密码，只显示账号密码是否已缓存或本次是否已填写。
...
在 Gradio 前端进入“预约座位”页面：

1. 添加一组常用预约信息。
2. 输入模板名称。
3. 点击“保存当前列表为模板”。
4. 下次选择模板后点击“加载模板”，模板里的预约会追加到当前预约列表。

说明：模板文件保存在本地 `cache/templates.json`，只保存学号、座位、区域和时间，不保存密码。加载模板后，如果账号没有本地密码缓存，预热或开始预约时仍会提示填写对应账号密码。
...
cache/history.json      # 前端历史预约输入记录
cache/templates.json    # 前端预约模板记录
